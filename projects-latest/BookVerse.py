...
def libr():
    global window
    window.withdraw()
    global win, bo,wo,tr, b1, b2, b3, b4
    win = Tk()
    win.title('Library')
    win.geometry("700x700+480+180")
    win.resizable(False, False)
    wo = Button(win,height=2,width=25,text='Ask question',command=quest)
    w1= Button(win,height=2,width=25,text='Generate random question',command=randomquest)
    ...
    wo.place(x=110,y=430)
    w1.place(x=110,y=480)
    bo.place(x=110, y=30)
    b1.place(x=110, y=80)
...
def connectdb():
    ...
    con = p.connect(host="localhost", user="root", passwd="S4soum@y")
    cur = con.cursor()
